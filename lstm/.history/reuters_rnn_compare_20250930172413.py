...
def load_reuters_dataset(
    ...
    """Load Reuters robustly across TF/Numpy versions.

    Strategy:
    1) Try TF Keras with allow_pickle when available.
    2) If signature doesn't support or numpy throws, manually load .npz with allow_pickle and filter by num_words.
    """
    try:
        (x_train, y_train), (x_test, y_test) = keras.datasets.reuters.load_data(
            num_words=max_features, allow_pickle=True
        )
    except TypeError:
        # Older TF-keras: fall back to manual loader to avoid numpy allow_pickle issue
