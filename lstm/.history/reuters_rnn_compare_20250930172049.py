...
def load_reuters_dataset(
    ...
    """Load Reuters with compatibility for numpy allow_pickle issues."""
    try:
        # Newer TF Keras exposes allow_pickle
        (x_train, y_train), (x_test, y_test) = keras.datasets.reuters.load_data(
            num_words=max_features, allow_pickle=True
        )
    except TypeError:
        # Older signature without allow_pickle
        (x_train, y_train), (x_test, y_test) = keras.datasets.reuters.load_data(
            num_words=max_features
        )
    except ValueError as err:
        # Work around: manually download and load with allow_pickle=True
        msg = str(err)
